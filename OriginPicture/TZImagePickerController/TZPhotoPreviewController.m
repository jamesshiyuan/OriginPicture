...
- (void)configCustomNaviBar {
   ...
   // _naviBar.backgroundColor = [UIColor colorWithRed:(34/255.0) green:(34/255.0)  blue:(34/255.0) alpha:1.0];
    UIImage *bg = [UIImage imageNamed:@"topBar.png"];
    UIImageView *bgView =[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.view.tz_width, 64)];
    [bgView setImage:bg];
    [_naviBar addSubview:bgView];
    _naviBar.alpha = 0.7;
...
- (void)configBottomToolBar {
    ...
    _okButton.frame = CGRectMake(self.view.tz_width/2 - 44 - 12, 0, 44, 44);
    _okButton.titleLabel.font = [UIFont systemFontOfSize:18];
    [_okButton addTarget:self action:@selector(okButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [_okButton setTitle:@"分享" forState:UIControlStateNormal];
...
- (void)showShareActionSheet:(UIView *)view photo:(NSArray<UIImage *> *)photos array:(NSArray *)assets  photo:(BOOL) isSelectOriginalPhoto
{
     ...
     **/
    __weak TZPhotoPreviewController *theController = self;
